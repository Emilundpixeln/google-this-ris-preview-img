...
async function search(query, options = {}) {
  ...
  const by_finger_print = options.by_finger_print || false;
  ...
  if (typeof query === 'object' && ris) {
    ...
    const url = encodeURI(
      ris ?
        (by_finger_print ?  `${Constants.URLS.W_GOOGLE}search?tbs=${_query}&ie=UTF-8&aomd=1${(safe ? '&safe=active' : '')}&start=${page}` : `${Constants.URLS.W_GOOGLE}searchbyimage?image_url=${_query}`) :
  ...
  results.finger_print = typeof query === 'object' && ris ? /tbs=(sbi:[^&]*)/g.exec(response.request.res.responseUrl)[1] : undefined;
